        ...
        Alamofire.request(DribbbleRestAPI.ListPopularShots(page)).responseSwiftyJSON { (request, response, json, error) -> Void in
            if (error != nil) {
                ...
                for (index: String, shotJson:JSON) in json["shots"] {
                    ...
                    var shot = Shot(id: shotJson["id"].intValue, title: shotJson["title"].stringValue, viewsCount: shotJson["views_count"].intValue, player: player, imageUrl: shotJson["imageUrl"].stringValue)
